...
const char * const PIVX_CONF_FILENAME = "belle.conf";
const char * const PIVX_PID_FILENAME = "belle.pid";
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\belle
// Windows >= Vista: C:\Users\Username\AppData\Roaming\belle
// Mac: ~/Library/Application Support/belle
// Unix: ~/.belle
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Belle";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\belleParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\belleParams
    // Mac: ~/Library/Application Support/belleParams
    // Unix: ~/.belle-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "belleParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "belleParams";
#else
    // Unix
    return pathRet / ".belle-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty belle.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override belle.conf
